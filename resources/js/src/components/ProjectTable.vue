...
import { ref } from 'vue';
import { GetProjectType, ProjectType } from '../pages/admin/project/action/getProject';
...
<template>
    ...
    <div class="flex flex-wrap">
        <!-- <div class="w-1/5 pr-2 pb-2" v-for="project in projects?.data?.data" :key="project.id">
            <RouterLink :to="'/kaban?query=' + project.slug" 
                class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
            >
                <p class="mb-2 font-bold tracking-tight text-gray-900 dark:text-white">{{ project.name }}</p>
            </RouterLink>
        </div> -->

        <div class="w-1/5 pr-2 pb-2">
            <a href="#" class="block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                <p class="mb-2 font-bold tracking-tight text-gray-900 dark:text-white">Noteworthy technology 2021</p>
            </a>
        </div>
    </div>
</template>